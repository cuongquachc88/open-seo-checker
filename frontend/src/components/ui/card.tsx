...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-xl border bg-card text-card-foreground shadow-soft transition-shadow hover:shadow',
        className,
      )}
      {...props}
    />
...
const CardStat = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'flex flex-col gap-1 px-6 py-5 border-b md:border-b-0 md:border-r last:border-r-0',
        className,
      )}
      {...props}
    />
